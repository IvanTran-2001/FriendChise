...
- `POST /api/orgs/[orgId]/tools/scan-to-task/upload-url` — `MANAGE_TASKS`, accepts `{ fileName, mimeType }`, returns `{ signedUrl, path }`, and reports `400`, `401`, `403`, or `500` when signing fails.
