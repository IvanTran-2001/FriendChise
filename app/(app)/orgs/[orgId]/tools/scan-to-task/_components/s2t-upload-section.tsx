...
export function ScanToTaskUploadSection({
  ...
  return (
    <section className="flex w-full min-w-0 flex-col rounded-2xl border border-border/60 bg-card p-4 sm:p-6">
      <UploadHeader />
      ...
      <form ref={formRef} onSubmit={onSubmit} className="mt-4 flex flex-col gap-4">
        ...
        <UploadFooter selectedFiles={selectedFiles} scanPending={scanPending} />
      </form>
    </section>
  );
}
...
function SelectedFilesList({ selectedFiles }: SelectedFilesListProps) {
  if (selectedFiles.length === 0) return null;

  return (
    <div className="flex min-w-0 flex-col gap-2">
      {selectedFiles.map((file) => (
        <span key={`${file.name}-${file.size}`} className="block w-full min-w-0 rounded-full border border-border bg-muted/40 px-3 py-1 text-xs text-muted-foreground wrap-break-word">
...
function UploadFooter({ selectedFiles, scanPending }: UploadFooterProps) {
  return (
