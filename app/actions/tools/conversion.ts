...
export async function createConversionSetAction(orgId: string, name: string) {
	...
	try {
		...
		const mappedError = mapPrismaError(err, { P2002: "A set with that name already exists." });
...
export async function deleteConversionSetAction(orgId: string, id: string) {
	...
	try {
		...
		const mappedError = mapPrismaError(err, { P2025: "Set not found." });
...
export async function createToolItemAction(orgId: string, name: string, unit: string) {
	const auth = await requireOrgPermissionAction(orgId, PermissionAction.MANAGE_TASKS);
	if (!auth.ok) return { ok: false as const };
	...
	try {
		...
		const mappedError = mapPrismaError(err, { P2002: "An item with that name already exists." });
		return { ok: false as const, error: mappedError ?? "Failed to create item." };
	}
}

export async function updateToolItemAction(orgId: string, id: string, name: string, unit: string) {
	const auth = await requireOrgPermissionAction(orgId, PermissionAction.MANAGE_TASKS);
	if (!auth.ok) return { ok: false as const };
...
export async function createConversionRateAction(
	...
	if (!Number.isFinite(fromQty) || fromQty <= 0)
		return { ok: false as const, error: "From quantity must be a finite number greater than 0." };
	if (!Number.isFinite(toQty) || toQty <= 0)
		return { ok: false as const, error: "To quantity must be a finite number greater than 0." };
	if (fromItemId === toItemId)
		return { ok: false as const, error: "From and To items must be different." };
...
export async function deleteConversionRateAction(orgId: string, setId: string, rateId: string) {
	...
	try {
		...
		const mappedError = mapPrismaError(err, { P2025: "Rate not found." });
...
export async function updateConversionRateAction(
	...
	if (!Number.isFinite(fromQty) || fromQty <= 0) return { ok: false as const, error: "From quantity must be > 0." };
	if (!Number.isFinite(toQty) || toQty <= 0) return { ok: false as const, error: "To quantity must be > 0." };
...
export async function createConversionTemplateAction(orgId: string, setId: string, name: string) {
	const auth = await requireOrgPermissionAction(orgId, PermissionAction.MANAGE_TASKS);
	if (!auth.ok) return { ok: false as const };
...
export async function deleteConversionTemplateAction(orgId: string, setId: string, templateId: string) {
	const auth = await requireOrgPermissionAction(orgId, PermissionAction.MANAGE_TASKS);
	if (!auth.ok) return { ok: false as const };
...
export async function deleteTemplateEntryAction(orgId: string, templateId: string, itemId: string) {
	const auth = await requireOrgPermissionAction(orgId, PermissionAction.MANAGE_TASKS);
	if (!auth.ok) return { ok: false as const };
