...
export default async function LogsPage({
  ...
  searchParams: Promise<{ search?: string | string[]; date?: string | string[]; page?: string | string[] }>;
}) {
  ...
  const [logs, totalCount] = await Promise.all([
    getAuditLogs("", { search, date, limit: PAGE_SIZE, page: currentPage }),
    getAuditLogsCount("", { search, date }),
  ]);
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1">
          {logs.length === 0 ? (
            ...
            <>
              ...
              <LogsPagination currentPage={currentPage} totalPages={totalPages} />
            </>
          )}
        </div>
      </div>
